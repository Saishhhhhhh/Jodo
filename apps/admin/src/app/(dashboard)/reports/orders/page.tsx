...
export default function OrdersReportPage() {
  ...
  const { data: summary, isLoading } = useQuery({
    queryKey: ['reports', 'summary', 'orders', dateRange],
    queryFn: async () => {
  ...
  const generateReportMutation = useMutation({
    mutationFn: () => reportsApi.generate({ ...dateRange, type: 'orders' }),
    onSuccess: (res) => {
    ...
    <div className="p-6 animate-fade-in space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Generate and download order status metrics.
          </p>
        </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          <CardTitle className="text-lg font-medium">Orders Overview (Preview)</CardTitle>
          ...
          {isLoading ? (
            ...
            <div className="flex gap-12 mt-4">
              <div>
                ...
                <div className="text-3xl font-bold mt-1 text-green-600 dark:text-green-500">{summary?.orders?.byStatus?.paid || 0}</div>
              </div>
              <div>
                <p className="text-sm text-muted-foreground">Total Pending</p>
                <div className="text-3xl font-bold mt-1 text-yellow-600 dark:text-yellow-500">{summary?.orders?.byStatus?.pending || 0}</div>
              </div>
            </div>
          )}
        </CardContent>
