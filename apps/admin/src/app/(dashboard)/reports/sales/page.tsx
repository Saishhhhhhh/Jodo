...
export default function SalesReportPage() {
  ...
  const { data: summary, isLoading } = useQuery({
    queryKey: ['reports', 'summary', 'sales', dateRange],
    queryFn: async () => {
  ...
  const generateReportMutation = useMutation({
    mutationFn: () => reportsApi.generate({ ...dateRange, type: 'sales' }),
    onSuccess: (res) => {
      toast.success('Sales report generated successfully');
      // In a real app we might redirect to /reports/view/[id] or just trigger download
    ...
    <div className="p-6 animate-fade-in space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Generate and download sales performance metrics.
          </p>
        </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          <CardTitle className="text-lg font-medium">Sales Overview (Preview)</CardTitle>
          ...
          {isLoading ? (
            ...
            <div className="flex gap-12 mt-4">
              ...
              <div>
                ...
                <div className="text-3xl font-bold mt-1">{summary?.sales?.totalOrders || 0}</div>
              </div>
            </div>
          )}
        </CardContent>
