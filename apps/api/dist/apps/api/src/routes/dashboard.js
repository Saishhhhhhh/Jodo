...
router.get('/summary', async (req, res) => {
    try {
        ...
        const store = await Store_1.Store.findOne({ tenantId, _id: storeId });
        // Fetch all store orders, active products, and returns
        const orders = await Order_1.Order.find({ tenantId, storeId }).sort({ createdAt: -1 });
        const returns = await Return_1.Return.find({ tenantId, storeId });
        ...
        const productCount = await Product_1.Product.countDocuments({ tenantId, storeId });
        const paymentPlugin = await AppPlugin_1.AppPlugin.findOne({ tenantId, storeId, status: 'active', name: /Razorpay|Stripe|Paypal/i });
        ...
        const pendingCount = orders.filter(o => o.fulfillmentStatus === 'unfulfilled' || o.fulfillmentStatus === 'partial').length;
        ...
        const summaryData = {
            ...
            ordersToday: {
                ...
                trend: ordersTodayCount > 0 ? 'up' : 'flat',
                format: 'number',
            },
            averageOrderValue: {
        ...
        (0, response_1.sendSuccess)(res, summaryData);
    }
    catch (err) {
        console.error(err);
