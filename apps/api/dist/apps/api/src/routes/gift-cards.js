...
router.get('/', async (req, res, next) => {
    try {
        const cards = await GiftCard_1.GiftCard.find({
            tenantId: req.auth.tenantId,
            storeId: req.auth.storeId,
        }).sort({ createdAt: -1 });
        (0, response_1.sendSuccess)(res, cards);
    }
    catch (error) {
        next(error);
    }
...
router.post('/', async (req, res, next) => {
    try {
        ...
        if (!initialValue || parseFloat(initialValue) <= 0) {
            return (0, response_1.sendError)(res, 'Initial value must be greater than 0', 400);
        }
        ...
        if (!finalCode) {
            ...
            while (!isUnique && attempts < 5) {
                ...
                const existing = await GiftCard_1.GiftCard.findOne({ storeId: req.auth.storeId, code: finalCode });
                if (!existing) {
                    isUnique = true;
                }
                attempts++;
            ...
            const existing = await GiftCard_1.GiftCard.findOne({ storeId: req.auth.storeId, code: finalCode });
            if (existing) {
                return (0, response_1.sendError)(res, 'Gift card code already exists', 400);
            }
        }
        ...
        (0, response_1.sendSuccess)(res, giftCard, 'Gift card issued successfully', 201);
    }
    catch (error) {
        next(error);
    }
...
router.put('/:id', async (req, res, next) => {
    try {
        ...
        const card = await GiftCard_1.GiftCard.findOne({
            _id: id,
            tenantId: req.auth.tenantId,
            storeId: req.auth.storeId,
        });
        if (!card) {
            return (0, response_1.sendError)(res, 'Gift card not found', 404);
        }
        if (status)
            card.status = status;
        if (balance !== undefined)
            card.balance = parseFloat(balance);
        if (note !== undefined)
            card.note = note;
        if (expiryDate !== undefined)
            card.expiryDate = expiryDate ? new Date(expiryDate) : undefined;
        await card.save();
        (0, response_1.sendSuccess)(res, card, 'Gift card updated successfully');
    }
    catch (error) {
        next(error);
    }
...
router.delete('/:id', async (req, res, next) => {
    try {
        const { id } = req.params;
        const result = await GiftCard_1.GiftCard.deleteOne({
            _id: id,
            tenantId: req.auth.tenantId,
            storeId: req.auth.storeId,
        });
        if (result.deletedCount === 0) {
            return (0, response_1.sendError)(res, 'Gift card not found or access denied', 404);
        }
        (0, response_1.sendSuccess)(res, null, 'Gift card revoked successfully');
    }
    catch (error) {
        next(error);
    }
