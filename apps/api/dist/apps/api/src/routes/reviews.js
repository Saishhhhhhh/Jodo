...
router.get('/', async (req, res, next) => {
    try {
        const reviews = await Review_1.Review.find({
            ...
            .sort({ createdAt: -1 });
        (0, response_1.sendSuccess)(res, reviews);
    }
    catch (error) {
        next(error);
    }
...
router.delete('/:id', async (req, res, next) => {
    try {
        const { id } = req.params;
        const result = await Review_1.Review.deleteOne({
            _id: id,
            tenantId: req.auth.tenantId,
            storeId: req.auth.storeId,
        });
        if (result.deletedCount === 0) {
            return (0, response_1.sendError)(res, 'Review not found or access denied', 404);
        }
