...
router.get('/collections/:slug', async (req, res, next) => {
    try {
        const collection = await Collection_1.Collection.findOne({ slug: req.params.slug, status: 'active' })
            .populate({
            path: 'products',
            match: { status: 'active' }
        });
...
router.get('/products/:id', async (req, res, next) => {
    try {
        const product = await Product_1.Product.findById(req.params.id).populate('addons');
        if (!product)
...
router.get('/products/:id/reviews', async (req, res, next) => {
    try {
        const reviews = await Review_1.Review.find({ productId: req.params.id, status: 'approved' }).sort({ createdAt: -1 });
