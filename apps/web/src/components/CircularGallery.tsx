...
class Media {
  ...
  update(scroll: { current: number; last: number }, direction: 'right' | 'left') {
    ...
    if (this.bend === 0) {
      // Shift up by half the text height so the entire card+text block is vertically centered
      const textHeightScaled = this.plane.scale.y * 0.15;
      this.plane.position.y = textHeightScaled * 0.5;
...
class App {
  ...
  addEventListeners() {
    ...
    if (typeof ResizeObserver !== 'undefined') {
      this.ro = new ResizeObserver(() => this.boundOnResize());
